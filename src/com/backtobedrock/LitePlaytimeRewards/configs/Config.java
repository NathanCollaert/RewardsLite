...
public class Config {
    ...
    public void setConfig(FileConfiguration fc) {
        ...
        this.rewards.clear();
        this.getRewards();
        this.plugin.getLogger().log(Level.INFO, "Loaded {0} rewards.", this.rewards.size());
    }
    ...
    public Material getBorderMaterial() {
        return this.getMaterial(this.config, "BorderMaterial", this.plugin.isLegacy() ? Material.matchMaterial("thin_glass") : Material.WHITE_STAINED_GLASS_PANE);
    }
    // </editor-fold>

    // <editor-fold desc="Playtime Options" defaultstate="collapsed">
    public boolean isCountAllPlaytime() {
        boolean countAll = this.config.getBoolean("CountAllPlaytime", true);
    ...
    private Material getMaterial(ConfigurationSection section, String id, Material material) {
        String configMaterial = section.getString(id, material.toString());
        Material mat = Material.matchMaterial(configMaterial);
        if (mat == null) {
            this.plugin.getLogger().log(Level.SEVERE, "{0} was not an existing material for {1}.", new Object[]{configMaterial, id});
            return material;
        }
        return mat;
    }
